...
let%template[@mode local] unsafe_bin_read_bytes buf ~pos ~len = exclave_
...
let%template[@mode local] bin_read_string buf ~pos_ref = exclave_
...
let%template[@mode local] bin_read_n_rev_list bin_read_el buf ~pos_ref len = exclave_
...
let%template[@mode local] bin_read_n_list bin_read_el buf ~pos_ref len = exclave_
...
let%template (check_array_len @ portable)
  (bin_read_el : (_ reader[@mode local]))
  ~len
  ~start_pos
  =
...
let (bin_read_array @ portable) (type a) (bin_read_el : _ reader) buf ~pos_ref =
...
let%template[@mode local] bin_read_iarray bin_read_el buf ~pos_ref = exclave_
  let len = (bin_read_nat0 buf ~pos_ref :> int) in
  check_array_len bin_read_el ~len ~start_pos:!pos_ref;
  Base.Iarray.Local.init len ~f:(fun _ -> exclave_ bin_read_el buf ~pos_ref)
;;
...
let%template[@mode local] unsafe_bin_read_md5 buf pos = exclave_
...
let[@mode local] bin_read_option bin_read_el buf ~pos_ref = exclave_
  let pos = safe_get_pos buf pos_ref in
  assert_pos pos;
  match unsafe_get buf pos with
  | '\000' ->
...
let[@mode local] bin_read_pair bin_read_a bin_read_b buf ~pos_ref = exclave_
  let a = bin_read_a buf ~pos_ref in
  let b = bin_read_b buf ~pos_ref in
  a, b
;;

let[@mode local] bin_read_triple bin_read_a bin_read_b bin_read_c buf ~pos_ref = exclave_
  let a = bin_read_a buf ~pos_ref in
  let b = bin_read_b buf ~pos_ref in
  let c = bin_read_c buf ~pos_ref in
  a, b, c
