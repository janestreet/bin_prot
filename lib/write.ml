...
let bin_write_int buf ~pos n =
  if n >= 0 then
    bin_write_int_nat0 buf ~pos n
  else
    bin_write_int_negative buf ~pos n
...
let bin_write_float32_vec buf ~pos v =
  ...
  let pos = bin_write_int_nat0 buf ~pos len in
  let size = len * 4 in
...
let bin_write_float32_mat buf ~pos m =
  ...
  let pos = bin_write_int_nat0 buf ~pos len1 in
  let pos = bin_write_int_nat0 buf ~pos len2 in
